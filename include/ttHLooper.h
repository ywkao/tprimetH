...
void add_variables(vector<Process*> v, TString tag, vector<TString> syst_labels = {""}) {
  ...
  for (unsigned int j = 0; j < syst_labels.size(); j++) {
      for (int i = 0; i < v.size(); i++) {
        ...
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_m600_tmva_origin30"     , 30  , -1. , 1.);
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_m600_tmva_origin100"    , 100 , -1. , 1.);
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_mixed03_tmva_origin30"  , 30  , -1. , 1.);
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_mixed03_tmva_origin100" , 100 , -1. , 1.);
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_mixed04_tmva_origin30"  , 30  , -1. , 1.);
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_mixed04_tmva_origin100" , 100 , -1. , 1.);
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_mixed05_tmva_origin30"  , 30  , -1. , 1.);
        v[i]->add_histogram("h" + syst_labels[j] + "MVA_value_smh_varset8_mixed05_tmva_origin100" , 100 , -1. , 1.);

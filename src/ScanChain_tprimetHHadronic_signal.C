...
int ScanChain_tprimetHHadronic_signal(TChain* chain, TString name_output_file, TString treeName, TString xml_file, TString mYear, bool fast = true, int nEvents = -1, string skimFilePrefix = "test") {
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) {
    ...
    for (unsigned int event = 0; event < nEventsTree; ++event)
    {
      ...
      if (evaluate_mva) mva_value_smh_varset8_mixed03_tmva_bdtg_original = mva_smh_varset8_mixed03_tmva_bdtg->EvaluateMVA( "BDTG" );
      if (evaluate_mva) mva_value_smh_varset8_mixed04_tmva_bdtg_original = mva_smh_varset8_mixed04_tmva_bdtg->EvaluateMVA( "BDTG" );
      if (evaluate_mva) mva_value_smh_varset8_mixed05_tmva_bdtg_original = mva_smh_varset8_mixed05_tmva_bdtg->EvaluateMVA( "BDTG" );
      ...
      if (evaluate_mva) mva_value_smh_varset8_mixed03_tmva_bdtg          = convert_tmva_to_prob(mva_value_smh_varset8_mixed03_tmva_bdtg_original);
      if (evaluate_mva) mva_value_smh_varset8_mixed04_tmva_bdtg          = convert_tmva_to_prob(mva_value_smh_varset8_mixed04_tmva_bdtg_original);
      if (evaluate_mva) mva_value_smh_varset8_mixed05_tmva_bdtg          = convert_tmva_to_prob(mva_value_smh_varset8_mixed05_tmva_bdtg_original);

...
void makeHeaderFile(TFile *f, const string& treeName, bool paranoid, const string& Classname, const string& nameSpace, const string& objName)
{
  ...
  for (Int_t i = 0; i < aliasarray->GetSize(); ++i) {
    ...
    for (unsigned int j = 0; j < vDuplicates.size(); j++) {
      if (vDuplicates[j] == aliasname) {
        cout << vDuplicates[j] << " is a duplicate branch, skipping repeated instance of it." << endl;
        duplicate = true;
      }
    }
    if (duplicate) continue;
    vDuplicates.push_back(aliasname);
